#!/usr/bin/env python3
"""
Steven x Theon v6 — robust Qwen3 retry + sanity checks
======================================================
Changes from v5:
  - Sanity check on every Qwen3 response: if clone duration > 3x slot OR > 20s
    absolute, reject and retry (Qwen3 occasionally produces 90s+ garbage for
    short inputs — caused the 1:00+ scrambled audio in v5)
  - Retry up to 3 attempts with cache-busting seed variations
  - Only cache the clone if it passes sanity check (no more cached rot)
  - Read timeout bumped to 60s; first request gets full timeout; retries 30s
  - Reuses v5 cache directory so 23 already-good clones aren't re-synthesized
"""
...
OUT_DIR = POC_DIR / "output_steven_v6"
CACHE_DIR = POC_DIR / "output_steven_v5" / "tts_cache"  # reuse v5's 23 good clones
...
def main():
    t0 = time.time()
    print("="*70); print(" STEVEN x THEON v6 — robust Qwen3 retry + sanity"); print("="*70)
    ...
    final_path = OUT_DIR / "steven_theon_personalized_v6.mp3"
