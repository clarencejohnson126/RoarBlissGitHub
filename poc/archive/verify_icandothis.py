#!/usr/bin/env python3
"""
Verify personalization pipeline outputs for 'I CAN DO THIS' for timeline drift and audio energy.
"""
...
def verify():
    output_dir = Path("/Users/clarence/Desktop/Roar Bliss App/poc/output_icandothis")
    original_vocals_path = output_dir / "vocals.wav"
    personalized_vocals_path = output_dir / "vocals_personalized.wav"
    final_output_path = output_dir / "final_output.mp3"

    print("\n" + "="*60)
    print("           ROAR BLISS 'I CAN DO THIS' VERIFICATION")
    print("="*60)
    ...
    if drift == 0:
        print("✓ SUCCESS: ZERO TIMELINE DRIFT! The audio is mathematically perfectly aligned!")
    elif drift < 50:
        print(f"✓ SUCCESS: Micro-drift of {drift} ms (less than 50ms) is completely imperceptible!")
    else:
        print(f"✗ ERROR: Timeline drift of {drift} ms detected. Timing is misaligned.")
        return False
    ...
    if final_output_path.exists():
        ...
        print(f"✗ ERROR: Final assembled MP3 does not exist yet.")
        return False
